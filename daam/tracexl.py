...
class PipelineHooker(ObjectHooker[StableDiffusionXLPipeline]):
    ...
    # def _hooked_run_safety_checker(hk_self, self: StableDiffusionXLPipeline, image, *args, **kwargs):
    ...
    def _hooked_encode_prompt(hk_self, _: StableDiffusionXLPipeline, prompt: Union[str, List[str]], *args, **kwargs):
        #print(prompt)
        # TODO: fix this 
        ...
        if 'negative_prompt' in kwargs.keys():
            if not isinstance(kwargs['negative_prompt'], str) and len(kwargs['negative_prompt']) > 1:
                ...
                last_negative_prompt = kwargs['negative_prompt'][0]
            else:
                last_negative_prompt = kwargs['negative_prompt']
        else:
    ...
    def _hooked_encode_prompt_total(hk_self, _: StableDiffusionXLPipeline, prompt: Union[str, List[str]], *args, **kwargs):
        #print(prompt)
        # TODO: fix this 
        if not isinstance(prompt, str) and len(prompt) > 1:
            raise ValueError('Only single prompt generation is supported for heat map computation.')
